...
class SplineSolver(SolverBase):
    ...
    def __post_init__(self):
        self.spline_basis = splines.get_basis(self.D, num_nodes=self.num_spline_nodes)
    ...
    def solve(self, energy_fn, x0, x1, init_params, num_final_points=None):
        ...
        if self.max_iter == 0:
            ...
            xs = splines.compute_spline(x=x0[:self.D], y=x1[:self.D], basis=self.spline_basis, params=init_params, ts=ts)
        ...
        def F(params):
            xs = splines.compute_spline(x=x0[:self.D], y=x1[:self.D], basis=self.spline_basis, params=params, ts=ts)
            condition_repeated = jnp.tile(x0[self.D:].reshape(1, -1), (xs.shape[0], 1))
            xs = jnp.concatenate([xs, condition_repeated], axis=-1)
            E = energy_fn(xs)
            return E
        ...
        lr_schedule = optax.cosine_decay_schedule(self.init_lr, decay_steps=self.max_iter, alpha=1e-1)
        opt = optax.adam(lr_schedule, b1=0.9, b2=0.999)
        opt_state = opt.init(init_params)
        spline_solver_state = SplineState(i=0, params=init_params, opt_state=opt_state, E=jnp.inf, grad_norm=jnp.inf)
        spline_solver_state = lax.stop_gradient(lax.while_loop(cond_fun, body_fun, spline_solver_state))
