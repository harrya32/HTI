...
class MetricFlowMatcher(ConditionalFlowMatcher):
    ...
    def compute_mu_t(self, x0, x1, t, t_min, t_max, cond=None):
        ...
        with torch.enable_grad():
            ...
            self.geopath_net_output = self.geopath_net(x0, x1, t, cond=cond)
            if self.geopath_net.time_geopath:
                self.doutput_dt = self.doutput_dt_fun(self.geopath_net, x0, x1, t, cond=cond)
        return (
    ...
    def sample_xt(self, x0, x1, t, epsilon, t_min, t_max, cond=None):
        mu_t = self.compute_mu_t(x0, x1, t, t_min, t_max, cond=cond)
    ...
    def sample_location_and_conditional_flow(
        ...
        xt = self.sample_xt(x0, x1, t, eps, t_min, t_max, cond=cond)
    ...
    @staticmethod
    def doutput_dt_fun(model, x0, x1, t_raw, cond=None):
        def f(tt):
            t_padded = pad_t_like_x(tt, x0)        
            return model(x0, x1, t_padded, cond=cond)
