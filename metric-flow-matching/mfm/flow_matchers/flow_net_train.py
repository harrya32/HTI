...
class FlowNetTrainBase(pl.LightningModule):
    ...
    def forward(self, t, xt, cond=None):
        return self.flow_net(t, xt, cond=cond)
    ...
    def _process_flow(self, x0s, x1s, c0s=None, c1s=None):
        ...
        for i, (x0, x1) in enumerate(zip(x0s, x1s)):
            ...
            if self.class_conditioning and self.categorical and c0s is not None:
                ...
                for cls in unique_classes:
                    ...
                    t_cls, xt_cls, ut_cls = self.flow_matcher.sample_location_and_conditional_flow(
                        x0_cls, x1_cls, t_start, t_start_next, cond=cond_cls
                    )
                    batch_ts.append(t_cls)
            ...
            elif self.class_conditioning and not self.categorical and c0s is not None:
                # Continuous conditioning: pair randomly, use x0's condition
                c0 = c0s[i]
                t_i, xt_i, ut_i = self.flow_matcher.sample_location_and_conditional_flow(
                    x0, x1, t_start, t_start_next, cond=c0
                )
                ts.append(t_i)
                xts.append(xt_i)
                uts.append(ut_i)
...
class FlowNetTrainTrajectory(FlowNetTrainBase):
    def test_step(self, batch, batch_idx):
        ...
        # Split condition from spatial data if conditioning
        if self.class_conditioning:
            ambient_dim = self.ambient_dim
            x0_full = batch[self.skipped_time_points[0] - 1] if self.skipped_time_points else batch[0]
            x0_spatial = x0_full[:, :ambient_dim]
            x0_cond = x0_full[:, ambient_dim:]
            if self.categorical:
                x0_cond = x0_cond.squeeze(-1)
            wrapper = flow_model_torch_wrapper(self.flow_net, cond=x0_cond)
        else:
            wrapper = flow_model_torch_wrapper(self.flow_net)
        ...
        if t_exclude is not None:
            if self.class_conditioning:
                start_data = batch[t_exclude - 1][:, :ambient_dim]
            else:
                start_data = batch[t_exclude - 1]
            ...
            if self.class_conditioning:
                target_spatial = batch[t_exclude][:, :ambient_dim]
            else:
                target_spatial = batch[t_exclude]

            EMD = wasserstein_distance(X_mid_pred, target_spatial, p=1)
            self.final_EMD = EMD
