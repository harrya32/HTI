...
def main(args: argparse.Namespace, seed: int, t_exclude: int) -> None:
    ...
    elif args.data_type == "custom_pt":
        pass  # no special assertions

    skipped_time_points = [t_exclude] if t_exclude else []

    ### DATAMODULES
    if args.data_type in ["arch", "scrna", "sphere", "custom_pt"]:
    ...
    class_conditioning = getattr(args, 'class_conditioning', False)
    categorical = getattr(args, 'categorical', True)
    cond_dim = getattr(args, 'cond_dim', 1)
    num_categories = getattr(datamodule, 'num_categories', None) if class_conditioning else None

    if args.data_type in ["arch", "scrna", "lidar", "sphere", "custom_pt"]:
    ...
    ot_sampler = (
        ...
        if args.optimal_transport_method != "None" and not class_conditioning
        else None
    )
    ...
    # Human-readable run ID for checkpoint directories
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_id = f"{timestamp}_{args.data_name}_s{seed}_{wandb.run.id}"
    ...
    if args.mfm:
        ...
        geopath_callbacks = create_callbacks(
            args, phase="geopath", data_type=args.data_type, run_id=run_id
        )
