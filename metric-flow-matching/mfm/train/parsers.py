...
def datasets_parser(parser):
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Time-step indices to select from .pt data (e.g. 0 5 -1)",
    )
    parser.add_argument(
        "--class_conditioning",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Enable class/condition-based FiLM conditioning from extra data dimensions",
    )
    parser.add_argument(
        "--categorical",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Whether the condition is categorical (True) or continuous (False)",
    )
    parser.add_argument(
        "--cond_dim",
        type=int,
        default=1,
        help="Dimensionality of the conditioning variable (columns after --dim in .pt data)",
    )
    parser.add_argument(
        "--instance_paired",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Keep instances aligned across timepoints (required when each instance has a unique condition)",
    )
    parser.add_argument(
        "--max_instances",
        type=int,
        default=None,
        help="Maximum number of instances per timepoint to load from .pt data (None = all)",
    )
    return parser
