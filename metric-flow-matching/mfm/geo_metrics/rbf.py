...
class RBFNetwork(pl.LightningModule):
    def __init__(
        ...
        self.ambient_dim = ambient_dim  # if set, strip condition cols beyond this index

    def _strip_cond(self, x):
        """Strip condition columns, keeping only spatial (ambient) dimensions."""
        if self.ambient_dim is not None and x.shape[-1] > self.ambient_dim:
    ...
    def on_train_start(self):
        with torch.no_grad():
            if self.image_data:
                ...
                all_data = self._strip_cond(torch.cat(metric_samples_batch_filtered))
    ...
    def training_step(self, batch, batch_idx):
        if self.image_data:
            ...
            inputs = self._strip_cond(torch.cat(
                [
                    x
    ...
    def validation_step(self, batch, batch_idx):
        ...
        h = self.forward(self._strip_cond(torch.cat(metric_samples_batch_filtered)))
        loss = ((1 - h) ** 2).mean()
        self.log(
