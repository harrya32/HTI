...
AGENT_PATH_4 = "ppo_reacher_weight_4.zip"
AGENT_PATH_5 = "ppo_reacher_weight_5.zip"
...
VEC_ENV_FILE_4 = "vec_normalize_reacher_weight_4.pkl"
VEC_ENV_FILE_5 = "vec_normalize_reacher_weight_5.pkl"
...
def pushforward(action, obs, lambda_val, workspace):
    ...
    time_points = 1 + np.array(workspace.time_points) * 4 # Scale to 1-5 range used in Reacher env
    time_points = np.array([int(tp) for tp in time_points])

    
    lower_mask = time_points <= lambda_val
    if np.any(lower_mask):
        nearest_lower_idx = np.where(lower_mask)[0][-1]
        nearest_lower_time = time_points[nearest_lower_idx]
    else:
        # handle case: all time_points > lambda_val
        nearest_lower_idx = None
        nearest_lower_time = None
...
def evaluate_lambda(model, vec_env_file, lambda_val, workspace, endpoint_model, endpoint_vec_env_file):
    ...
    if lambda_val in END_POINTS:
        env = VecNormalize.load(endpoint_vec_env_file, DummyVecEnv([make_reacher_env]))
    else:
...
endpoint_to_model = {1: (model_1, VEC_ENV_FILE_1),
                     2: (model_2, VEC_ENV_FILE_2),
                     3: (model_3, VEC_ENV_FILE_3),
                     4: (model_4, VEC_ENV_FILE_4),
                     5: (model_5, VEC_ENV_FILE_5)}
...
for workspace_file in workspace_files:
    ...
    for lambda_val in LAMBDA_VALUES:
        ...
        endpoint_model = endpoint_to_model[lambda_val][0]
        endpoint_vec_env_file = endpoint_to_model[lambda_val][1]

        avg_penalty, std_dev, avg_reward = evaluate_lambda(model, vec_env_file, lambda_val, workspace, endpoint_model, endpoint_vec_env_file)
