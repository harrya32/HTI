...
def pushforward(action, obs, lambda_val, workspace):
    ...
    time_points = np.array(workspace.time_points)
...
def evaluate_lambda(model, lambda_val, workspace, endpoint_model):
...
if args.all_lambdas:
    ...
    for workspace_file in workspace_files:
        ...
        for lambda_val in LAMBDA_VALUES:
            model = lambda_to_model[lambda_val]
            endpoint_model = endpoint_to_model.get(lambda_val, None)
            avg_penalty, std_dev, avg_reward = evaluate_lambda(model, lambda_val, workspace, endpoint_model)
